...
def _sexbabesvr_info(slug):
    resp = _fetch(f"https://sexbabesvr.com/video/{slug}/")
    if resp is None or resp.status_code != 200:
        return None, None
    m = re.search(r'"uploadDate":\s*"(\d{4})-(\d{2})-(\d{2})', resp.text)
    if not m:
...
def _tmwvrnet_info(slug):
    resp = _fetch(f"https://tmwvrnet.com/trailers/{slug}.html")
    if resp is None or resp.status_code != 200:
        return None, None
    m = re.search(r'<meta\s+property="video:release_date"\s+content="(\d{4})-(\d{2})-(\d{2})"', resp.text)
    if not m:
        return slug, None
    year, dd, mm = m.group(1), m.group(2), m.group(3)
    return slug, f"{year}{mm}{dd}"


def _virtualrealporn_info(slug):
    resp = _fetch(f"https://virtualrealporn.com/vr-porn-video/{slug}/")
    if resp is None or resp.status_code != 200:
        return None, None
...
def _virtualtaboo_info(slug):
    if slug.endswith("-files"):
        slug = slug[:-6]
    resp = _fetch(f"https://virtualtaboo.com/videos/{slug}")
    if resp is None or resp.status_code != 200:
        return None, None
    final_slug = resp.url.rstrip("/").rsplit("/", 1)[-1]
    m = re.search(r'"uploadDate":"(\d{4})-(\d{2})-(\d{2})', resp.text)
    if not m:
        return final_slug, None
    return final_slug, f"{m.group(1)}{m.group(2)}{m.group(3)}"
...
def _vrcosplayx_info(slug):
    slug_us = slug.replace("-", "_")
    resp = _fetch(f"https://vrcosplayx.com/cosplaypornvideo/{slug_us}/")
    if resp is None or resp.status_code != 200:
        return None, None
    final_slug = resp.url.rstrip("/").rsplit("/", 1)[-1]
    m = re.search(r'itemprop="uploadDate"\s+content="(\d{4})-(\d{2})-(\d{2})', resp.text)
    if not m:
        return final_slug, None
    return final_slug, f"{m.group(1)}{m.group(2)}{m.group(3)}"
...
def _vrixxens_info(slug):
    resp = _fetch(f"https://vrixxens.com/{slug}/")
    if resp is None or resp.status_code != 200:
        return None, None
    m = re.search(r'"datePublished":"(\d{4})-(\d{2})-(\d{2})', resp.text)
    if not m:
        return slug, None
    return slug, f"{m.group(1)}{m.group(2)}{m.group(3)}"


def _vrlatina_info(slug):
    resp = _fetch(f"https://vrlatina.com/video/{slug}.html")
    if resp is None or resp.status_code != 200:
        return None, None
    m = re.search(r'Release date:\s*</div>\s*<span[^>]*>\s*([A-Z][a-z]+)\s+(\d{1,2}),\s+(\d{4})', resp.text, re.DOTALL)
    if not m:
        return slug, None
    mon = MONTH_MAP.get(m.group(1).lower())
    if mon is None:
        return slug, None
    return slug, f"{m.group(3)}{mon:02d}{int(m.group(2)):02d}"
...
STUDIO_FETCHERS = {
    ...
    "stasyqvr": _slr_info,
    "tmwvrnet": _tmwvrnet_info,
    "virtualrealporn": _virtualrealporn_info,
    "virtualtaboo": _virtualtaboo_info,
    "vrallure": _slr_info,
    "vrbangers": _vrbangers_info,
