    ...
    try:
        ...
        if HAS_BS4:
            ...
            if not iframe_tag or not iframe_url:
                iframe_tag = soup.select_one(
                    'iframe[src*="fypttstr.php"], iframe[src*="fypttjwstrhls.php"]'
                )
                if iframe_tag:
                    iframe_url = iframe_tag.get("data-src-no-ap") or iframe_tag.get(
                        "src"
                    )
        elif HAS_LXML:
            doc = html.fromstring(response.text)
            ...
            if not iframe_elements:
                iframe_elements = doc.cssselect(
                    'iframe[src*="fypttstr.php"], iframe[src*="fypttjwstrhls.php"]'
                )
            if iframe_elements:
                iframe_url = iframe_elements[0].get(
                    "data-src-no-ap"
                ) or iframe_elements[0].get("src")
        else:
            ...
            if match:
                ...
                if match:
                    ...
                    pattern = (
                        r'<iframe[^>]*src="([^"]*fyptt(?:jw)?str(?:hls)?\.php[^"]*)"'
                    )
                    match = re.search(pattern, response.text)
                    if match:
                        iframe_url = match.group(1)
        ...
        if HAS_BS4:
            ...
            source_tag = iframe_soup.select_one(
                'source[type="video/mp4"], source[src*=".m3u8"], source[src*=".mp4"]'
            )
            if source_tag:
                video_download_url = source_tag.get("src")
                video_type = "m3u8" if ".m3u8" in (video_download_url or "") else "mp4"
            if not video_download_url:
                video_tag = iframe_soup.select_one(
                    'video[src*=".mp4"], video[src*=".m3u8"]'
                )
                if video_tag:
                    video_download_url = video_tag.get("src")
                    video_type = (
                        "m3u8" if ".m3u8" in (video_download_url or "") else "mp4"
                    )
        elif HAS_LXML:
